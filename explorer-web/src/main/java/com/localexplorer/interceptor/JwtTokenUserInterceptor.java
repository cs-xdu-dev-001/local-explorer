...
public class JwtTokenUserInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            requireClaim(claims, JwtClaimsConstant.PRINCIPAL_TYPE, AuthSessionServiceImpl.USER);
            Long userId = Long.valueOf(claims.get(JwtClaimsConstant.USER_ID).toString());
            String sessionId = claims.get(JwtClaimsConstant.SESSION_ID, String.class);
            if (sessionId == null || !authSessionService.isActive(sessionId, AuthSessionServiceImpl.USER, userId)) {
                throw new IllegalStateException("用户会话已失效");
            }
            User user = userMapper.getById(userId);
            if (user == null || !StatusConstant.ENABLE.equals(user.getStatus())) {
                throw new IllegalStateException("用户账号已禁用或不存在");
            }
            BaseContext.setCurrentId(userId);
            request.setAttribute(JwtClaimsConstant.SESSION_ID, sessionId);
            log.debug("JWT校验通过，当前用户id：{}", userId);
            return true;
        } catch (Exception ex) {
